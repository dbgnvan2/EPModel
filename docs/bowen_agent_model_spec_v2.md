...
| | Count |
|---|---|
| New IDs | **89** — M1 4, M2 1, M3 5, M4 6, M6 1, M7 1, M9 1, M10 4, M11 23, M13 1, M14 3, M15 1, M16 9, M17 29 |
...
| New acceptance criteria | **6** in `M11.C` (`M11.C.35`–`M11.C.40`) and **8** in `M11.D` (`M11.D.15`–`M11.D.22`) |
...
| 6 | C19's corpus ordering | Not understood | **Open.** In plain terms: an ordinal criterion needs the corpus to rank effects — mechanism X changes the outcome more than Y, and Y more than Z. `M11.4c` allows that kind of test, but no such ranking has been found, so nothing uses it yet. The question is whether you know of one |
| 7 | The directions of `M11.C.35` and `M11.C.36` | Both confirmed | No change |
| 8 | `E-RM` within `M0.4` (`DESIGN_LESSONS` Q10) | Not answered | **Open**; `TODO.md` |
...
| 11 | `E-DR` against the constant sweep (Q8) | Asked for a recommendation | Recommendation applied, for confirmation: keep the two separate, because they answer different questions, but make `M17.E.2` sweep the same range `M17.E.1` samples |
...
| Check | Result |
|---|---|
| New criteria numbered from `M11.C.35` (revision 9's table ended at `M11.C.34`) | pass — `M11.C.35`–`M11.C.40`; `M11.D` continues at `M11.D.15`–`M11.D.22` |
| Every ID matches `M\d+(\.[A-Za-z0-9]+)*`; no duplicates | pass — 516 defined IDs, 0 failures, 0 duplicates |
| Revision 9's IDs unchanged | pass — all 427 still defined, 0 removed, 89 added; extracted from both versions with the same pattern and diffed |
...
| Every marker resolves | pass — 133 markers; every part names a C-number heading in the candidates file, an `E-DR`/`E-SH`/`E-RM`/`E-RE` draft in `DESIGN_LESSONS` §7.8–§7.9, or a `DESIGN_LESSONS` section or §7.10 item, except three administrative forms (*owner decision 2026-09-22*, *first draft, rev10*, *register extension*) and the *reader's proposal* and *recommended rev10* tags on `M3.D.4b`'s rows |
...
| §0.3's counts | restated above: 828 tokens, 78 named tests (revision 9: 663, 51). The project's `M11.D.14` test passes against them |
...
| Requirement count cited by other documents | **fails, and is left failing on this branch.** The project's requirement-count test finds `CLAUDE.md`, `README.md`, `CHANGELOG.md`, `docs/theory/_STATUS.md` and `agent_model_proposal.html` claiming 427 against the spec's 516. This revision may not edit `CLAUDE.md` or `docs/theory/`, and restating the others would present unapproved proposals as the count. Update all five at approval, with the count as approved |
