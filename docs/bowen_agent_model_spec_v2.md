...
**Phase E drafted at revision 10.** The sentence above that says Phase E is specified after the core is real is superseded in part: Phase E is now drafted in `M17` as a **first draft**, on the project owner's decision of 2026-09-22 that everything the method literature proposes should sit in this one document for review. `M13.3`'s rule applies to `M17` with one exception, `M13.4`: the adaptive stopping rule of `M17.A.1`, with `M17.A.3` and `M17.A.4`, is built in Phase C because Phases C and D use it. The owner has also decided that `M17` stays in this document rather than becoming a separate Phase E spec. Every requirement added at revision 10 carries a proposal marker until the owner accepts or strikes it. ⟦proposed rev10 · owner decision 2026-09-22⟧
...
| `papers/SPEC_CANDIDATES_from_preprints_2026-09-20.md` and `papers/DESIGN_LESSONS_model_design_papers_2026-09-17.md` | the evidence behind requirements marked `[proposed rev10]` — candidates C1–C42, and the design-lessons sections (`DL §…`) including the `E-DR`, `E-SH`, `E-RM` and `E-RE` drafts | **The evidence, not the rationale.** Their role is the ledger's: the rationale is mirrored in `model_explainer.md` §18 on the owner's decision of 2026-09-22, and the spec cites `→ model_explainer.md §18 · C8` the way it cites `→ §9.4` for the corpus. ⟦proposed rev10 · owner decision 2026-09-22⟧ |
...
**M1.B.13** A tie's identifier **MUST** be the unordered pair of its members' `M1.A.20` identifiers. → `model_explainer.md` §18 · C2 ⟦proposed rev10 · C2 · Buffalo 2026⟧
...
**M1.C.7** A triangle's identifier **MUST** be the sorted triple of its members' `M1.A.20` identifiers. → `model_explainer.md` §18 · C2 ⟦proposed rev10 · C2 · Buffalo 2026⟧
...
**M1.F.1b** — *the witness set is computed, not chosen.* An Event's `witnesses` **MUST** be computed by the visibility component (`M3.E.1`) from tie and household state — co-residence, tie conductance and route — and **MUST NOT** be a field the sender's policy sets. Where the theory needs a sender to choose an audience, the choice **MUST** be expressed as a move (`TRIANGLE` addressed to a third party), not as witness selection. `M8.5`'s predicate, which decides whether a third party is a witness or a new peripheral triangle, belongs to that component. → `model_explainer.md` §18 · C7 ⟦proposed rev10 · C7 · Li & Tao 2026⟧
...
**M2.A.0h** The reference family's declaration **MUST** assign each member the fixed identifier `M1.A.20` requires. Display names **MUST NOT** serve as identifiers. → `model_explainer.md` §18 · C2 ⟦proposed rev10 · C2 · Buffalo 2026⟧
...
| Channel mixing-weight noise, if any | `M4.D.1a` | tick, actor, purpose, index | slot — the mix is part of what A does this week, so it is coupled exactly as move selection is, with its own purpose label ⟦proposed rev10 · C3 · Buffalo 2026; recommended rev10⟧ |
...
| Per-edge latency, where stochastic | `M1.B.11`, `M3.C.1` | tick, actor, partner, purpose, index | dyad — latency belongs to the edge (`M1.B.11`), so a different receiver is a different chance event ⟦proposed rev10 · C3 · Buffalo 2026; recommended rev10⟧ |
...
| Mortality | `M7.C.1`, `M6.3` | slow tick, person, purpose | per person — as for symptom onset: an arm with a higher hazard makes the same person's death the same or earlier, a per-seed monotone coupling ⟦proposed rev10 · C3 · Buffalo 2026; recommended rev10⟧ |
| Tie-break and fallback | `M4.D.1f` | tick, actor, purpose, index | slot, with a purpose label distinct from selection — a tie arising in one arm and not the other then shifts no other draw ⟦proposed rev10 · C3 · Buffalo 2026; recommended rev10⟧ |

**M3.D.4c** The engine **MUST** query each event key at most once per run and cache the value, and a debug build **MUST** raise on a repeated key. `M11.D.15` is the test. → `model_explainer.md` §18 · C4 ⟦proposed rev10 · C4 · Buffalo 2026⟧
...
**M4.D.1e** — *the legal set is formed before selection.* For each person each fast tick, the policy **MUST** compute the set of legal outcomes from current tie and triangle state — the structural preconditions of each move (for example, `CUTOFF` requires a live tie, `TRIANGLE` a reachable third party, `PURSUE` a target not cut off) and every `M5.C` gate that removes a move from the draw — and **MUST** select only over that set, with weights renormalised over it. A gate that degrades a move rather than removing it (`M5.C.1`'s `outside_ness` row) is not part of the mask. The mask is logged (`M16.A.3b`). → `model_explainer.md` §18 · C11 ⟦proposed rev10 · C11 · Buitrago López 2026⟧

**M4.D.1f** — *tie-break and fallback are declared, keyed and flagged.* The policy **MUST** declare a tie-break rule for equal scores and a fallback rule for a non-finite score, exhausted `life_energy` and an empty legal set. Both rules are `[I]` and **MUST** be declared in config; any draw either makes **MUST** use a key declared in `M3.D.4b`. Every selection record **MUST** state whether its outcome came from the scored policy, the tie-break or the fallback (`M16.A.3c`), because a fallback rule can generate the very behaviour a criterion tests. `M11.D.18` is the guard. → `model_explainer.md` §18 · C12 ⟦proposed rev10 · C12 · Ye 2026⟧
...
**M4.E.1a** The `witnesses` field of the event `M4.E.1` creates **MUST** be filled by the visibility component (`M1.F.1b`), not by the policy that selected the move. → `model_explainer.md` §18 · C7 ⟦proposed rev10 · C7 · Li & Tao 2026⟧
...
**M7.C.1e** Mortality (`M7.C.1`) **MUST** remove a Person only through `M6.3`'s named mechanism. → `model_explainer.md` §18 · C15 ⟦proposed rev10 · C15 · He 2026⟧
...
- the two declared `basic_level` levels and two stress loads of `M11.C.41` — inputs, not calibrated magnitudes ⟦proposed rev10 · owner decision 2026-09-22⟧
...
**M10.C.5** — *a premise column.* The register **MUST** mark each rule and constant that `M11.1d`'s audit finds to be a programmed premise — one whose single inversion flips a criterion — so that a result a premise produces is not reported as derived. → `model_explainer.md` §18 · C22 ⟦proposed rev10 · C22 · Zhou 2026⟧
...
| any criterion under `M17.A.1`, in Phases C, D and E (`M13.4`) | a **third outcome**, `UNDETERMINED`, when the interval has not converged at the seed cap | It is a statement that the ensemble did not resolve the direction, not a claim about the model. δ and the cap are declared inputs, graded `[I]` ⟦proposed rev10 · C26 · Blando 2026⟧ |
...
**M11.4c** — *ordinal criteria, admitted as a form.* Where the corpus supplies an ordering of effect strengths across three or more mechanisms, a criterion **MUST** assert the rank order of the per-seed paired arm differences across those mechanisms, not only the sign of each. It passes when the observed ordering matches the corpus ordering in a pre-declared majority of seeds, and deleting the mechanism ranked first **MUST** break the ordering. The ordering is an input taken from the corpus and the majority is declared and graded `[I]`; neither is a calibrated magnitude. No criterion of this form is instantiated at revision 10. The owner's answer of 2026-09-22 is that most mechanisms scale with the same two inputs — differentiation and stress — so the theory supplies orderings of **conditions**, which `M11.C.41` tests as ordinary two-arm directions, rather than a ranking of mechanisms by strength. → `model_explainer.md` §18 · C19 ⟦proposed rev10 · C19 · Kalluri 2026⟧

**M11.4d** — *a null at a bound is an assay limit.* Every acceptance test **MUST** record the baseline arm's position within the readout's attainable range, and a null on a readout whose baseline sits at a bound **MUST** be reported as an assay limit, not as evidence about the mechanism. `M11.4a` covers the underpowered null; this covers the saturated one. → `model_explainer.md` §18 · C24 ⟦proposed rev10 · C24 · Prasad 2026⟧

**M11.4e** — *the statistic is defined for degenerate arms, and multiplicity is declared.* The direction test **MUST** be defined when one arm has zero seed-to-seed variance or discrete bounded outcomes — a rank-based paired statistic on the per-seed differences — and where several readouts are tested under one criterion, a multiplicity correction **MUST** be declared. Under `M3.D.4a`'s coupled arms the test is on per-seed differences, never an unpaired comparison of the two arms. → `model_explainer.md` §18 · C25 ⟦proposed rev10 · C25 · Ye 2026⟧

**M11.4f** — *a comparison with a mechanism switched off is admissible, but only as a test of the model's structure.* Settled on 2026-09-22, on a recommendation the owner adopted (`DESIGN_LESSONS` Q10): switching a mechanism off is a change to the model, and such comparisons remain allowed as acceptance tests. `M0.4`'s differencing is trustworthy because both arms share one parameter set, so the invented constants cancel between them (`M11.F.9(c)`). With a mechanism disabled on one side, the comparison is between two models, and the constants that interact with that mechanism no longer cancel. Such a comparison — `M17.D.2`'s rival-mechanism arm, the controls of `M17.D.1` that suppress a mechanism, and the learning switch of `M10.B.5` on which `M11.C.39` and `M11.C.40` rest — **MUST** therefore be reported as a test of the model's structure (whether the credited mechanism is needed), **MUST NOT** be reported as the effect of an intervention on a family, and in Phase E **MUST** be run across `M17.E.1`'s constant sweep before its direction is relied on. It also reaches the approved ablation of `M10.C.4a`, by the owner's decision of 2026-09-22: each run with a quantum-jump condition removed tests whether the model needs that condition, which is what `M10.C.4a` is for, so nothing in that requirement changes. `M11`'s mutation protocol is consistent with it already: a mutant is treated as a different model whose only job is to show a test can fail. ⟦proposed rev10 · owner decision 2026-09-22; E-RM · SEAA⟧
...
| **M11.C.39** | **A pattern learned under a spell outlasts it.** An exogenous anxiety spell (`M1.F.6`) establishes a relational pattern — raised `DISTANCE` frequency, conductance loss, a `CUTOFF`. Two arms, identical seeds, differing only in whether the automatic channel learns (`M10.B.5`): after the spell ends, the pattern **MUST** persist longer in the learning arm than in the learning-off arm, because `DISTANCE` and `CUTOFF` remove the contact that would disconfirm the learned response. No persistence rule is written; persistence is emergent. The report **MUST** show the time-to-remission distribution per seed. Learning off is a change to the model (`M11.4f`), so this is a test of structure ⟦proposed rev10 · C18 · Prasad 2026⟧ | `test_m11c39_pattern_outlasts_spell_only_with_learning` | D | `M4.D.6` — disabling reinforcement in both arms **MUST** turn this red |
| **M11.C.40** | **Reinforcement narrows the automatic channel only.** Two arms, identical seeds, differing only in whether the automatic channel learns (`M10.B.5`). The learning arm **MUST** show a larger fall over the run in automatic-channel repertoire entropy (`M16.A.9`) than the learning-off arm, and the between-arm difference in self-directed-channel entropy **MUST** be smaller than the between-arm difference in automatic-channel entropy, since `M4.D.6d` forbids reinforcing the self-directed channel. The entropy describes concentration, not differentiation. Learning off is a change to the model (`M11.4f`), so this is a test of structure ⟦proposed rev10 · E-RE · SEAA⟧ | `test_m11c40_leaked_reinforcement_raises_self_directed_entropy` | D | `M4.D.6d` — letting reinforcement reach the self-directed channel **MUST** turn this red |
| **M11.C.41** | **Lower differentiation and higher stress each exacerbate the pattern.** Four arms, identical seeds, crossing `basic_level` (a lower and a higher declared level) with the stressor schedule (a lighter and a heavier declared load). At each level, the heavier-stress arm **MUST** show higher mean acute anxiety and a higher share of the seven reactive moves (`M4.D.3`) than the lighter-stress arm; at each stress load, the lower-level arm **MUST** show higher values on both than the higher-level arm. The two off-diagonal arms — low level under light stress, high level under heavy stress — are **not** ordered against each other, because that depends on magnitudes the corpus does not give. The readout **MUST NOT** be overt emotionality, which peaks mid-scale (`M1.A.3c`). These are four two-arm directions, so no `M11.4` exception is needed. It extends `M11.C.1` from one input to two, and is the owner's answer to the question C19 raised: the theory orders **conditions** by differentiation and stress rather than ranking mechanisms against each other. → owner (domain expert), 2026-09-22; `M1.C.3a`; `M11.C.1` ⟦proposed rev10 · owner decision 2026-09-22; C19 · Kalluri 2026⟧ | `test_m11c41_level_and_stress_each_exacerbate_the_pattern` | C | the `/ functional_level` divisor in `M4.C.1`, and `M4.D.3`'s anxiety weighting — removing either **MUST** turn at least one of the four directions red |
...
**M11.D.22** — *the engine cannot see which arm it is in (`M17.D.3`).* A static check **MUST** fail if any policy, appraisal or consolidation module imports from a test or readout module. It has the same shape as `M11.D.12` and runs from Phase B, on the owner's decision of 2026-09-22. `test_m11d22_engine_modules_do_not_import_tests_or_readouts` ⟦proposed rev10 · C35 · Zhou 2026; owner decision 2026-09-22⟧
...
**M13.4** — *amended: Phases C and D stop adaptively.* This replaces "over 1,000-seed ensembles" in Phase C's Done-when cell, and the ensembles Phase D's cell requires, with `M17.A.1`'s adaptive rule, on the owner's decision of 2026-09-22 that a fixed count lets ensemble size, not the mechanism, decide marginal results. `M17.A.1`, `M17.A.3` and `M17.A.4` **MUST** therefore be built in Phase C, as an exception to `M13.3`, and a criterion reported `UNDETERMINED` at the seed cap **MUST NOT** count as passing its phase. → `model_explainer.md` §18 · C26 ⟦proposed rev10 · C26 · Blando 2026; owner decision 2026-09-22⟧
...
**M14.A.1** The object model (`M1`) **MUST** be accompanied by a register in which every state variable of `Person`, `Relationship`, `Triangle` and `Family` is classified — exogenous-homogeneous, exogenous-heterogeneous, endogenous-decision or endogenous-derived — with the mechanisms that write it. → `model_explainer.md` §18 · C10 ⟦proposed rev10 · C10 · He 2026⟧

**M14.A.2** Every mechanism **MUST** list what it reads, what it writes on its owner and what it writes on other objects, and `M3.D.1`'s phase order **MUST** list each mechanism with its execution mode — synchronous batch, latency-delivered or slow tick — and its trigger condition. `M1.F.8`'s same-tick batching is none of these modes and **MUST** be recorded as a documented composite. → `model_explainer.md` §18 · C10 ⟦proposed rev10 · C10 · He 2026⟧

**M14.A.3** The register below is a **stub**. Its rows are for the owner to fill when the implementation plan is written; `M11.D.17` checks it. → `model_explainer.md` §18 · C10 ⟦proposed rev10 · C10 · He 2026⟧
...
**M15.A.1a** The stable id of `M15.A.1` **MUST** be the identifier `M1.A.20` uses, and **MUST** be carried unchanged into every arm run over the imported family. → `model_explainer.md` §18 · C2 ⟦proposed rev10 · C2 · Buffalo 2026⟧
...
**M16.A.1a** The header **MUST** also record the identity and version of the activation and visibility components (`M3.E.1`) and the activation regime in use (`M3.E.2`). → `model_explainer.md` §18 · C6 ⟦proposed rev10 · C6 · Li & Tao 2026⟧
...
**M16.A.3a** The selection record **MUST** also carry the belief value the policy used for each tie it read (`M4.B.2`, `M9.8`), so the renderer can show believed against true tie state. → `model_explainer.md` §18 · C9 ⟦proposed rev10 · C9 · He 2026⟧

**M16.A.3b** The selection record **MUST** carry the legal set formed under `M4.D.1e`, so that a move that was never legal is distinguishable from one that was legal and never chosen. → `model_explainer.md` §18 · C11 ⟦proposed rev10 · C11 · Buitrago López 2026⟧

**M16.A.3c** The selection record **MUST** carry a flag stating whether the outcome was chosen by the scored policy, the tie-break rule or the fallback rule (`M4.D.1f`). → `model_explainer.md` §18 · C12 ⟦proposed rev10 · C12 · Ye 2026⟧
...
**M16.A.5a** For each belief that has a true-state counterpart, the log **MUST** allow the signed discrepancy (belief − truth) to be computed per tick. → `model_explainer.md` §18 · C41 ⟦proposed rev10 · C41 · Kalluri 2026⟧
...
**M16.A.7** The header **MUST** record, for each `[I]` constant, whether its value was changed after the acceptance suite first ran against it (`M10.B.4`). → `model_explainer.md` §18 · C23 ⟦proposed rev10 · C23 · Zhou 2026⟧

**M16.A.8** For every mechanism with asymmetric `[I]` constants (`M11.1e`), the log **MUST** record per-event magnitudes and event counts separately, so that a cumulative asymmetry can be split into its rule and its frequency. → `model_explainer.md` §18 · C20 ⟦proposed rev10 · C20 · Kalluri 2026⟧

**M16.A.9** — *repertoire entropy, per channel.* The log **SHOULD** carry, per agent and per window, the normalised entropy of the distribution of selected moves over the `M5` repertoire, computed **separately** for the `AUTOMATIC` and `SELF_DIRECTED` channels from the channel `M1.F.1a` records on every event. It describes concentration and **MUST** be labelled as such: it is **not** a measure of differentiation, maturity or `basic_level`, and a falling value means behaviour has narrowed, not that anyone has matured. It needs no new state and no engine change — an observer computes it from the `M4.E.1` event stream — so `M16.B.3` holds. It is the only observable `M4.D.6d` has; `M11.C.40` is the test, and `M17.B.5`'s transition readout carries it as a per-channel column. → `model_explainer.md` §18 · DL §7.9 ⟦proposed rev10 · E-RE · SEAA⟧

**M16.A.10** For every `[I]` constant used as a threshold in policy or appraisal, the engine **SHOULD** emit, per tick, the signed margin of each comparison made against it. It is an observer-side record class, so `M16.B.3` holds. → `model_explainer.md` §18 · C31 ⟦proposed rev10 · C31 · Kurz 2025⟧
...
→ `model_explainer.md` §18 · C34; DL §2.6, §7.8 `E-RM` ⟦proposed rev10 · C34 · Sachdeva 2025; E-RM · SEAA⟧
...
This is distinct from `M10.C.4a`, which removes a condition from the jump mechanism; `M17.D.2` removes the mechanism and asks whether the observable survives without it. Disabling a mechanism is a change to the model, not an arm, by the owner's ruling (`M11.4f`), so this arm tests the model's structure and is reported under `M11.4f`. Tests: `test_m17d2_rival_arm_is_run_and_reported`, `test_m17d2_criterion_asserts_which_member_not_that_members_differ`, `test_m17d2_non_discriminating_criterion_is_reported_as_such`; an assertion that requires only non-zero divergence between members **MUST** turn the second red. → `model_explainer.md` §18 · DL §7.8 `E-RM` ⟦proposed rev10 · E-RM · SEAA; C34 · Sachdeva 2025⟧

**M17.D.3** — *the engine is arm-blind.* The engine **MUST NOT** receive an arm label, scenario name, test identifier or readout definition. Arms **MUST** differ only through declared channels: initial state, `[I]` constants, exogenous spells, and mechanism switches declared in `M10` (for example `M10.B.5`). The static import check that enforces this in the build is `M11.D.22`, which runs from Phase B. → `model_explainer.md` §18 · C35 ⟦proposed rev10 · C35 · Zhou 2026⟧

**M17.D.4** — *floors on null arms.* Wherever a mechanism-disabled arm exists (`M17.D.1`, `M17.D.2`, `M10.C.4a`), the criterion **SHOULD** also assert that the observable is **absent** in that arm, as a declared upper bound over the ensemble rather than a literal zero. A direction test passes when both arms show the phenomenon and one shows more; a floor test fails there. The bound is `[I]`. Floors stay a SHOULD, by the owner's decision of 2026-09-22 (`DESIGN_LESSONS` Q15). `test_m17d4_null_arm_observable_stays_below_declared_floor` → `model_explainer.md` §18 · DL §7.10(c) ⟦proposed rev10 · DL §7.10(c) · SEAA⟧

**M17.D.5** — *both signs of every perturbation.* Every perturbation arm **SHOULD** be run in both directions, raised and lowered, because a directional match on one side can hide an asymmetric failure on the other. → `model_explainer.md` §18 · DL §2.6 ⟦proposed rev10 · DL §2.6 · Yang 2024⟧

### M17.E Sweeps and sensitivity

**M17.E.1** — *the constant sweep, reported as a fraction of range.* For each `M11.C` criterion, Phase E **MUST** sample the `[I]` constants over declared ranges and report the fraction of samples in which the asserted direction holds. A criterion that holds only in a narrow band of invented constants is a weaker result than one that holds across the range, and a single-point run cannot tell them apart. The rate at which ties harden or attenuate, relative to the rate at which anxiety moves, **MUST** be swept as a named ratio. → `model_explainer.md` §18 · DL §2.3, §2.4 ⟦proposed rev10 · DL §2.3 · Castellano 2009; DL §2.4 · Castellano 2009⟧
...
Where a direction depends on a ratio of two `[I]` rates, the ratio **MUST** be named in `M10` and swept. A criterion whose dominant constant cannot be identified **MUST** be reported as not covered by this requirement, not silently omitted. This does not replace `M10.C.4a`. It is kept separate from `M17.E.1` because the two answer different questions — `M17.E.1` asks how much of the joint constant space a direction survives in, and `M17.E.2` asks whether one constant is necessary and where its default sits — but it **MUST** sweep the same declared range for its constant that `M17.E.1` samples, so the two reports can be read together. This arrangement was approved by the owner on 2026-09-22. Tests: `test_m17e2_null_value_removes_effect`, `test_m17e2_reports_monotonicity`, `test_m17e2_flags_rising_edge_operating_point`; pinning the sweep to one point, or reporting only the end-of-range value, **MUST** turn them red. → `model_explainer.md` §18 · DL §7.8 `E-DR` and its mapping table ⟦proposed rev10 · E-DR · SEAA; C16 · Prasad 2026; C24 · Prasad 2026; C31 · Kurz 2025⟧

**M17.E.3** — *threshold margins and the invariance interval.* For every `[I]` constant used as a threshold in policy or appraisal, Phase E **MUST** report, for each directional criterion, the interval of that constant within which every seed's trajectory is unchanged, bounded by the minimum positive and negative margins logged over the run (`M16.A.10`). `M10`'s register gains an **invariance interval** column. A comparison decided within numerical noise is flagged here; `M11.1c` is its counterpart. → `model_explainer.md` §18 · C31 ⟦proposed rev10 · C31 · Kurz 2025⟧

**M17.E.4** — *sensitivity at two or more reference configurations.* For each dimension audited under `M17.G.1`, sensitivity **MUST** be measured at no fewer than two declared reference configurations of the `[I]` constants — for instance a low- and a high-differentiation family — because a perturbation that is null at one configuration may flip the outcome at another. → `model_explainer.md` §18 · C37 ⟦proposed rev10 · C37 · Ye 2026⟧

**M17.E.5** — *pairwise additivity residual.* For parameter pairs the theory says interact — chronic anxiety × `basic_level`; functioning balance × conductance — Phase E **SHOULD** run a two-dimensional grid and report the maximum residual against the additive prediction from the two one-dimensional sweeps. → `model_explainer.md` §18 · C38 ⟦proposed rev10 · C38 · Prasad 2026⟧

**M17.E.6** — *an alternative activation regime.* Phase E **SHOULD** run the acceptance criteria under at least one activation regime other than `M3.E.2`'s default — seeded random-sequential selection, or a Poisson clock — by swapping the `M3.E.1` activation component, and report which directional results survive. No principled way to choose between regimes is known, so neither is preferred. → `model_explainer.md` §18 · DL §2.1 ⟦proposed rev10 · DL §2.1 · Axtell 2022⟧
...
`model_explainer.md` §18, on the owner's decision of 2026-09-22; the candidates and design-lessons files are
the evidence behind it (§0.2).
...
| | Count |
|---|---|
| New IDs | **91** — M1 4, M2 1, M3 5, M4 6, M6 1, M7 1, M9 1, M10 4, M11 25, M13 1, M14 3, M15 1, M16 9, M17 29 |
| Amendments of approved requirements, by suffixed ID | **3** — `M3.D.4a` (C1, replaces `M3.D.4`'s second sentence), `M11.4b` (the exceptions count) and `M13.4` (adaptive ensembles in Phases C and D) |
| New acceptance criteria | **7** in `M11.C` (`M11.C.35`–`M11.C.41`) and **8** in `M11.D` (`M11.D.15`–`M11.D.22`) |
| New `[I]` register entries | **18** in `M10.C.1a`, naming **25** constants; none is given a value |
| Candidates entered | **all 42** (C1–C42), the four §7.8 drafts, and all six §7.10 tests; C19 enters as a criterion form with no instance; `M11.C.41` is the owner's answer to it |
...
| `M10.C.1a` | one per entry — C6, C8, C12, C13, C16, C19, C21, C26, C30, C32, C37, E-RE, E-SH, E-DR, DL §2.3, §2.6, §7.10(c), §7.10(e), and the owner's decision for `M11.C.41` | as each entry's marker | register |
...
| `M11.4f` | owner decision 2026-09-22 (`DESIGN_LESSONS` Q10), E-RM | owner; Liu 2026, SEAA (2609.17331) | new |
...
| `M11.C.41` | owner decision 2026-09-22 (answer to C19) | owner, as domain expert; `M1.C.3a` | criterion |
...
| `M11.D.22` | C35 | Zhou et al. 2026, PIMMUR (2509.18052) | criterion |
| `M13.4` | C26 | Blando et al. 2026 (2604.04543) | amendment |
...
**The owner's answers of 2026-09-22, and what was done with each.** The fourteen questions this section
first listed, in their original order.

| # | Question | Answer | Done |
|---|---|---|---|
| 1 | Death: an exit from the field, or absorbed by the family? (C15, `M6.3`) | **Deferred for later review.** Guidance: relationship energy behaves like chemical bond energy and does not disappear | Guidance recorded in `M6.3` as not yet a requirement; `TODO.md` |
| 2 | Where method rationale lives | In `model_explainer.md` | Mirrored in explainer §18; every method citation in this spec now points there; §0.2 recasts the `papers/` files as evidence |
| 3 | `M17` in this document or split out | One master document | Recorded in §0.1 |
| 4 | C3's keys | Do the recommendation | The reader's six keys adopted as the recommendation; the four uncovered classes given recommended keys (`M3.D.4b`, marked *recommended rev10*) |
| 5 | C13's relief term | **Review later** | `M4.G.3` stays marked; `TODO.md` |
| 6 | C19's corpus ordering | Most mechanisms scale with differentiation of self and the stress being experienced: lower differentiation and higher stress mean more anxiety and more exaggerated patterns of functioning, such as more reactivity | New criterion `M11.C.41`, gated in Phase C: four two-arm directions over a 2×2 of level and stress. `M11.4c`'s mechanism ranking stays uninstantiated; strike it at approval unless a ranking turns up |
| 7 | The directions of `M11.C.35` and `M11.C.36` | Both confirmed | No change |
| 8 | `E-RM` within `M0.4` (`DESIGN_LESSONS` Q10) | Switching off a mechanism is a change; the owner then adopted the recommendation: such comparisons are allowed, as tests of the model only | New rule `M11.4f`: such comparisons are between two models, are reported as tests of structure, never as effects on a family, and are swept in Phase E. It reaches `M17.D.1`, `M17.D.2`, `M11.C.39` and `M11.C.40`, and — by the owner's decision — the approved ablation of `M10.C.4a`, which it leaves unchanged |
| 9 | `E-RM` and C34, one requirement or two (Q16) | **Needs review** | Left as `M17.D.1`(d) plus `M17.D.2`; `TODO.md` |
| 10 | Floors (Q15) | Keep as SHOULD | Recorded in `M17.D.4` |
| 11 | `E-DR` against the constant sweep (Q8) | Asked for a recommendation, then **approved** it | Kept separate, because they answer different questions; `M17.E.2` sweeps the same range `M17.E.1` samples |
| 12 | Ensemble size in Phases C and D | Adaptive | `M13.4` amends the fixed 1,000-seed ensembles; `M17.A.1`, `M17.A.3` and `M17.A.4` are built in Phase C, and `UNDETERMINED` does not pass a phase |
| 13 | The arm-blindness check from Phase B | Do the recommendation | `M11.D.22`, gated in Phase B |
| 14 | `CLAUDE.md` still says revision 6 and 427 | Yes, update after approval | `TODO.md`, together with the four other documents in the count check below |
...
| Check | Result |
|---|---|
| New criteria numbered from `M11.C.35` (revision 9's table ended at `M11.C.34`) | pass — `M11.C.35`–`M11.C.41`; `M11.D` continues at `M11.D.15`–`M11.D.22` |
| Every ID matches `M\d+(\.[A-Za-z0-9]+)*`; no duplicates | pass — 518 defined IDs, 0 failures, 0 duplicates |
| Revision 9's IDs unchanged | pass — all 427 still defined, 0 removed, 91 added; extracted from both versions with the same pattern and diffed |
| `M11.D.11`, both directions | pass — every `M11.C` criterion's phase matches its `M13` Done-when cell (B 0, C 19, D 21, E 1) and no cell names an orphan; the project's three `M11.D.11` tests pass |
| `M11.D.13`, no subject-negated `MUST` | pass — the project's test, and a separate scan of the added text |
| Every marker resolves | pass — 136 markers; every part names a C-number heading in the candidates file, an `E-DR`/`E-SH`/`E-RM`/`E-RE` draft in `DESIGN_LESSONS` §7.8–§7.9, or a `DESIGN_LESSONS` section or §7.10 item, except three administrative forms (*owner decision 2026-09-22*, *first draft, rev10*, *register extension*) and the *reader's proposal* and *recommended rev10* tags on `M3.D.4b`'s rows |
| No quotation of eight or more words from a paper | pass — every added line compared as eight-word sequences against the `pdftotext` output of all 70 PDFs under `papers/`, including the fourteen sweep papers and SEAA, and the same comparison over the new `model_explainer.md` §18: 0 matches |
| §0.3's counts | restated above: 835 tokens, 79 named tests (revision 9: 663, 51). The project's `M11.D.14` test passes against them |
...
| Requirement count cited by other documents | **fails, and is left failing on this branch.** The project's requirement-count test finds `CLAUDE.md`, `README.md`, `CHANGELOG.md`, `docs/theory/_STATUS.md` and `agent_model_proposal.html` claiming 427 against the spec's 518. This revision may not edit `CLAUDE.md` or `docs/theory/`, and restating the others would present unapproved proposals as the count. Update all five at approval, with the count as approved |
