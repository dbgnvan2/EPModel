...
| 8 | `E-RM` within `M0.4` (`DESIGN_LESSONS` Q10) | Switching off a mechanism is a change | New rule `M11.4f`: such comparisons are between two models, are reported as tests of structure, never as effects on a family, and are swept in Phase E. It reaches `M17.D.1`, `M17.D.2`, `M11.C.39` and `M11.C.40`; whether it reaches the approved `M10.C.4a` is for the owner |
