...
**M7.A.2a** — *open design question, flagged not decided.* Two independent cases argue `systems_perspective` (M1.A.18) should be **per-tie** rather than a global scalar. The spec currently makes it global. Resolve before Phase C; if it becomes per-tie, M1.E.7's rise applies to the tie the contact concerned, and M4.D.3b's gate is evaluated per tie.
